...
def extract_url_features(urls: pd.Series) -> pd.DataFrame:
    ...
    for url in urls:
        try:
            ...
            is_dev_tld = 1 if domain.endswith(('.dev', '.me', '.io', '.xyz', '.fyi', '.page')) else 0
...
def extract_structural_features(html_contents: pd.Series) -> pd.DataFrame:
    ...
    for html in html_contents:
        ...
        try:
            ...
            generator = generator_tag['content'].lower() if generator_tag else ''
            
            scripts = [s.get('src', '') for s in soup.find_all('script') if s.get('src')]
            has_hubspot = 1 if any('js.hs-scripts.com' in s for s in scripts) else 0
...
def extract_content_features(text_contents: pd.Series) -> pd.DataFrame:
    ...
    for text in text_contents:
        ...
        corporate_pronouns = len(re.findall(r'\b(we|our|company)\b', text_lower))
        corporate_keywords = len(re.findall(r'\b(solutions|enterprise|platform|services|b2b)\b', text_lower))
